...
// Main source file for Classic+ Controller from Hand Held Legend.
...
// Pin for toggling USB switch line
#define GPIO_USB_TOGGLE     GPIO_NUM_4
...
// This function will test, on boot, if L and R are being held to determine if
// we should enter USB Bootloader Update mode. 
void usb_switch_check()
{
    ...
    if (g_button_data.t_l && g_button_data.t_r)
    {
        ESP_LOGI("usb_switch_check", "Entering USB flash mode...");
    ...
    ESP_LOGI("usb_switch_check", "USB Mode not enabling.");
}
...
void app_main()
{
    ...
    io_conf.intr_type = GPIO_INTR_DISABLE;
    io_conf.pin_bit_mask = (1ULL << GPIO_USB_TOGGLE);
    io_conf.mode = GPIO_MODE_OUTPUT;
    io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
    io_conf.pull_up_en = GPIO_PULLUP_ENABLE;
    gpio_config(&io_conf);
    gpio_set_level(GPIO_USB_TOGGLE, 0);
