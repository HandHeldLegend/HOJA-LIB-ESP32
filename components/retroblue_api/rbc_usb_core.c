...
rb_err_t rbc_core_usb_start()
{
    const char* TAG = "rbc_core_usb_start";

    // Set up I2C peripheral.

	i2c_config_t conf = {};
    conf.mode = I2C_MODE_MASTER;
    conf.sda_io_num = RB_PIN_I2C_SDA;
    conf.scl_io_num = RB_PIN_I2C_SCL;
    ...
    if ( (response[0] == USB_CMD_SYSTEMSET) && (response[1] == USB_MSG_OK) )
    {
        ...
        // Start SNES task.
        xTaskCreatePinnedToCore(rbc_core_usb_sendinput_task, "USB I2C Task Loop", 2024,
...
void rbc_core_usb_sendinput_task(void * parameters)
{
    ...
    const char* TAG = "rbc_core_usb_sendinput_task";
